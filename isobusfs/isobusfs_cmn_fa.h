...
struct isobusfs_fa_openf_req {
	...
	uint8_t tan;
	uint8_t flags;
	__le16 path_name_length;
	// Dynamically allocated array to hold name
	uint8_t name[];
	//uint8_t name[ISOBUSFS_ETP_MAX_TRANSFER_SIZE - 5]; /* Name (Filename, Path or Wildcard name) */
};
...
struct isobusfs_read_dir_response {
	...
	__le16 count;
	uint8_t data[];
	//uint8_t data[ISOBUSFS_ETP_MAX_TRANSFER_SIZE - 5];
};
 ...
 */
struct isobusfs_close_file_response {
	uint8_t fs_function;
	uint8_t tan;
	uint8_t error_code;
	uint8_t reserved[6];
