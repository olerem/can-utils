...
int isobusfs_cli_ccm_send(struct isobusfs_priv *priv)
{
	...
	time_diff = isobusfs_timespec_diff_ms(&priv->cmn.next_send_time,
					      &priv->cmn.last_time);
	...
	isobusfs_timespec_add_ms(&priv->cmn.next_send_time, 2000);
...
static void isobusfs_cli_fs_activate(struct isobusfs_priv *priv)
{
	...
	priv->state = ISOBUSFS_CLI_STATE_CONNECTING_DONE;
}
