			 ...
			 size_t name_len)
{
	...
	req = malloc(req_len);
	if (!req) {
		pr_err("failed to allocate memory for current directory request");
		return -ENOMEM;
	}
...
static int isobusfs_srv_dh_current_dir_res(struct isobusfs_priv *priv,
			  ...
			  struct isobusfs_msg *msg)
{
	...
	case ISOBUSFS_DH_F_CHANGE_CURRENT_DIR_RES: /* fall through */
	default:
		goto not_supported;
	}
