				     ...
				     const char *path, int fd)
{
	...
	if (!file) {
		...
		file = &priv->files[handle];
	} else {
		handle = file - priv->files;
	}
...
static struct isobusfs_srv_files *
isobusfs_srv_get_file(struct isobusfs_srv_priv *priv, int handle)
{
	if (handle < 0 || handle >= ARRAY_SIZE(priv->files)) {
		return NULL;
	}

	return &priv->files[handle];
}
...
static int isobusfs_srv_release_file(struct isobusfs_srv_priv *priv,
				     ...
				     int handle)
{
	struct isobusfs_srv_files *file = isobusfs_srv_get_file(priv, handle);
	int client_index;
					   ...
					   struct isobusfs_srv_client *client)
{
	...
	for (handle = 0; handle < ARRAY_SIZE(priv->files); handle++) {
		struct isobusfs_srv_files *file = &priv->files[handle];
				  ...
				  struct isobusfs_msg *msg)
{
	...
	// Cast the message data to the close file request structure
	...
	file = isobusfs_srv_get_file(priv, req->handle);
	if (!file) {
				  ...
				  struct isobusfs_msg *msg)
{
	...
	ret = isobusfs_srv_release_file(priv, client, req->handle);
	if (ret < 0) {
		pr_warn("failed to release file with handle: %x", req->handle);
		switch (ret) {
		case -ENOENT:
			error_code = ISOBUSFS_ERR_FILE_ORPATH_NOT_FOUND;
			break;
