...
struct isobusfs_cm_fs_properties {
	uint8_t fs_function;
	uint8_t version;
...
struct isobusfs_cm_fs_status {
	uint8_t fs_function;
	uint8_t status;
	uint8_t opened_files;
	uint8_t reserved[5];
};
...
struct isobusfs_dh_current_dir_res {
	uint8_t fs_function;
	uint8_t tan;
	uint8_t error_code;
	...
	uint8_t path_name[ISOBUSFS_MAX_LONG_FILENAME_LENGH];
};
...
struct isobusfs_dh_get_current_dir_req {
	uint8_t fs_function;
	uint8_t tan;
	uint8_t reserved[6];
};
...
inline static int isobusfs_buf_to_cmd(uint8_t *buf)
{
	return (buf[0] & 0xf0) >> 4;
}

inline static int isobusfs_buf_to_function(uint8_t *buf)
{
	return (buf[0] & 0xf);
}

inline static uint8_t isobusfs_cg_function_to_buf(enum isobusfs_cg cg,
						   uint8_t func)
{
...
int64_t isobusfs_timespec_diff_ms(struct timespec *ts1, struct timespec *ts2);
