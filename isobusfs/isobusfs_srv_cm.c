				...
				struct isobusfs_srv_volume *volume)
{
	...
	for (j = 0; j < ARRAY_SIZE(volume->clients); j++) {
		if (volume->clients[j] == client) {
			// Client already requested this volume, do not increase refcount
			return 0;
		}
	}
				...
				const char *volume_name)
{
	...
	for (i = 0; i < priv->volume_count; i++) {
		if (strcmp(priv->volumes[i].name, volume_name) == 0) {
			...
			// Find the client in the volume's client list and decrease refcount
...
static void isobusfs_srv_remove_client_from_volumes(struct isobusfs_srv_priv *priv,
                                                 struct isobusfs_srv_client *client)
{
				       ...
				       struct isobusfs_srv_client *client)
{
	...
	isobusfs_srv_remove_client_from_files(priv, client);
...
void isobusfs_srv_remove_timeouted_clients(struct isobusfs_srv_priv *priv)
{
	...
	for (i = 0; i < priv->clients_count; i++) {
		...
		time_diff = isobusfs_timespec_diff_ms(&priv->cmn.last_time,
					...
					char *dst, size_t dst_size)
{
	...
	while (i < 3 && i < src_size && j < dst_size - 1)
		dst[j++] = src[i++];
		...
		struct isobusfs_msg *msg, struct isobusfs_cm_vol_stat_res *resp)
{
	...
	if (req->volume_mode & ISOBUSFS_VOL_MODE_PREP_TO_REMOVE) {
		if (!volume->removable ||
		    (req->name_len == 0 && 0 /* Current directory is not set condition */)) {
			// Volume is not removable, or the Path Name Length of request is zero and the current directory is not set
			return ISOBUSFS_ERR_INVALID_ACCESS;
		}
