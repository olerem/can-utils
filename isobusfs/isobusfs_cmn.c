...
void isobusfs_send_nack(int sock, struct isobusfs_msg *msg)
{
	...
	struct sockaddr_can local_addr = {0};
	...
	pr_debug("send NACK");
	localaddr_len = sizeof(local_addr);
	ret = getsockname(sock, (struct sockaddr*)&local_addr,
			  &localaddr_len);
	if (ret < 0) {
		ret = -errno;
		pr_warn("failed to get local address: %i (%s)",
			ret, strerror(ret));
		return;
	}

	memset(&nack, 0xff, sizeof(nack));
	pgn = msg->peername.can_addr.j1939.pgn & 0xffff;

	nack.ctrl = ISOBUS_ACK_CTRL_NACK;
	nack.pgn_nack = htole16(pgn);
	nack.address_nack = local_addr.can_addr.j1939.addr;
