				    ...
				    size_t x)
{
	...
	/* 49 characters max: 16 bytes in hexadecimal + 16 spaces + 2 "  " + 8
	 * characters + 1 null terminator
	 */
	char output[49];
	...
	for (size_t i = start_offset; i < buffer_size; i += 8) {
		...
		/* Print the offset */
		output_ptr += snprintf(output_ptr, sizeof(output), "%08lx: ",
				       (unsigned long)(start_offset + i));

		/* Print the bytes in hexadecimal form */
		for (int j = 0; j < 8 && i + j < buffer_size; ++j) {
			output_ptr += snprintf(output_ptr, sizeof(output) -
					       (output_ptr - output), "%02x ",
					       buffer[i+j]);
		}

		/* Insert additional spaces for alignment */
		for (int j = buffer_size - i; j < 8; ++j) {
			output_ptr += snprintf(output_ptr, sizeof(output) -
					       (output_ptr - output), "   ");
		}
