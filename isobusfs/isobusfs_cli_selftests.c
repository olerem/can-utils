...
// TODO: implement tests with other flags
...
size_t current_of_pattern_test = 0;
...
size_t current_sf_pattern_test = 0;
...
size_t current_rf_pattern_test = 0;
...
void isobusfs_cli_run_self_tests(struct isobusfs_priv *priv)
{
	if (!all_tests_completed) {
		...
		if (current_test < num_tests) {
			...
			if (!test_running) {
				pr_info("Executing test %zu: %s", current_test + 1, test_cases[current_test].test_description);
				test_running = true;
				priv->state = ISOBUSFS_CLI_STATE_SELFTEST;
			}
			...
			if (test_complete) {
				test_running = false;
				pr_info("Test %zu: %s.", current_test + 1, ret ? "FAILED" : "PASSED");
				current_test++;
				priv->state = ISOBUSFS_CLI_STATE_SELFTEST;
			}
		} else {
