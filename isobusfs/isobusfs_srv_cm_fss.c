...
{
	struct isobusfs_cm_fs_status *st = &priv->st;
	struct sockaddr_can *name = &priv->st_name;
	...
	name->can_family = AF_CAN;
	name->can_addr.j1939.addr = J1939_NO_ADDR;
	name->can_addr.j1939.name = J1939_NO_NAME;
	name->can_addr.j1939.pgn = ISOBUSFS_PGN_FS_TO_CL;
}
...
static unsigned int isobusfs_srv_fss_get_rate(struct isobusfs_srv_priv *priv)
{
	...
	case ISOBUSFS_SRV_STATE_BUSY:
		return ISOBUSFS_CM_F_FS_STATUS_BUSY_RATE;
	default:
		pr_warn("unknown state %d", priv->st_state);
	}
...
int isobusfs_srv_fss_send(struct isobusfs_srv_priv *priv)
{
	...
	time_diff = isobusfs_timespec_diff_ms(&priv->st_next_send_time,
					      &priv->last_time);
	...
	ret = isobusfs_sendto(priv->ctrl_sock, &priv->st, sizeof(priv->st),
			      &priv->st_name, &priv->tx_buf_log);
	if (ret < 0) {
	...
	pr_debug("> tx FS status: 0x%02x, opened files: %d",
		 priv->st.status, priv->st.opened_files);

	/* Calculate time for the next status message */
	next_msg_rate = isobusfs_srv_fss_get_rate(priv);
	priv->st_next_send_time = priv->last_time;
	isobusfs_timespec_add_ms(&priv->st_next_send_time, next_msg_rate);
